...
function pes_activate() {
	global $wpdb;

	$table_name      = $wpdb->prefix . 'pes_stats';
  ...
  CREATE TABLE $table_name (
    ...
    size ENUM('hr', 'web') NOT NULL DEFAULT 'hr',
    PRIMARY KEY  (id),
...
register_activation_hook( __FILE__, 'pes_activate' );


function pes_scripts() {
	wp_register_style( 'pes_funct_style', plugins_url( 'css/pes-functionalities.css', __FILE__ ), false, '1.0', 'all' );
	wp_register_script( 'chartjs', 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.5.0/Chart.bundle.min.js', array(), '2.5.0', true );
	wp_register_script( 'pes_funct_js', plugins_url( 'js/pes-functionalities.js', __FILE__ ), array() );

	if ( is_page( 'stats' ) ) {
		wp_enqueue_style( 'pes_funct_style' );
		wp_enqueue_script( 'chartjs' );
		wp_enqueue_script( 'pes_funct_js' );
	}
}

add_action( 'wp_enqueue_scripts', 'pes_scripts' );


function pes_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'type' => 'no foo',
	), $atts, 'pes_stats' );

	return pes_get_stats();
}

add_shortcode( 'pes_stats', 'pes_shortcode' );

function pes_get_stats() {
	global $wpdb;

	$key  = isset( $_GET['key'] ) ? urldecode( $_GET['key'] ) : 'downloads';
	$sort = isset( $_GET['sort'] ) ? urldecode( $_GET['sort'] ) : 'desc';

	// Force values to avoid injections.
	$sort          = ( $sort == 'desc' ) ? 'desc' : 'asc';
	$sort_inverted = ( $sort == 'desc' ) ? 'asc' : 'desc';
    ...
    SELECT 
      COUNT(*) as downloads, 
      s.attachment_id,
      a.post_title as attachment_title,
      a.post_name as attachment_name,
      a.guid as attachment_url,
      a.post_parent as gallery_id, 
      g.post_title as gallery_title, 
      g.post_name as gallery_name,
      s.size, 
      um.meta_value as country
    FROM " . $wpdb->prefix . "pes_stats s 
    INNER JOIN " . $wpdb->prefix . "posts a ON s.attachment_id = a.ID AND a.post_type = 'attachment' 
    INNER JOIN " . $wpdb->prefix . "posts g ON a.post_parent = g.ID AND g.post_type = 'gallery'
    LEFT JOIN " . $wpdb->prefix . "usermeta um ON s.user_id = um.user_id AND um.meta_key = 'country'
    INNER JOIN " . $wpdb->prefix . "usermeta umr ON s.user_id = umr.user_id AND umr.meta_key = 'role' 
      AND umr.meta_value NOT LIKE '%pes%' AND umr.meta_value NOT LIKE '%admin%'
    GROUP BY s.attachment_id, s.size, um.meta_value
    ORDER BY downloads $sort, gallery_title, attachment_title, size, um.meta_value
	" );
	/*
	SELECT COUNT(*) as downloads, s.attachment_id, a.post_title as attachment_title, a.post_name as attachment_name, a.guid as attachment_url, a.post_parent as gallery_id, g.post_title as gallery_title, g.post_name as gallery_name, s.size, um.meta_value as country, DATE_FORMAT(s.`timestamp`, '%Y-%m-%d') as dl_date FROM wp_ifxo_pes_stats s INNER JOIN wp_ifxo_posts a ON s.attachment_id = a.ID AND a.post_type = 'attachment' INNER JOIN wp_ifxo_posts g ON a.post_parent = g.ID AND g.post_type = 'gallery' LEFT JOIN wp_ifxo_usermeta um ON s.user_id = um.user_id AND um.meta_key = 'country' INNER JOIN wp_ifxo_usermeta umr ON s.user_id = umr.user_id AND umr.meta_key = 'role' AND umr.meta_value NOT LIKE '%pes%' AND umr.meta_value NOT LIKE '%admin%' GROUP BY s.attachment_id, s.size, um.meta_value ORDER BY downloads DESC, gallery_title, attachment_title, size, um.meta_value;
	*/

	$graph = $wpdb->get_results( "
    SELECT COUNT(*) as downloads, DATE_FORMAT(s.timestamp, '%d %b') as dl_date, s.size
    FROM " . $wpdb->prefix . "pes_stats s
    INNER JOIN " . $wpdb->prefix . "usermeta umr ON s.user_id = umr.user_id AND umr.meta_key = 'role' 
      AND umr.meta_value NOT LIKE '%pes%' AND umr.meta_value NOT LIKE '%admin%'
    WHERE s.timestamp > DATE_SUB(NOW() - INTERVAL 1 DAY, INTERVAL 1 MONTH)
    GROUP BY s.timestamp, s.size
    ORDER BY s.timestamp ASC
    " );
	/*
	SELECT COUNT(*) as downloads, DATE_FORMAT(s.timestamp, '%d %b') as dl_date, s.size FROM wp_ifxo_pes_stats s INNER JOIN wp_ifxo_usermeta umr ON s.user_id = umr.user_id AND umr.meta_key = 'role' AND umr.meta_value NOT LIKE '%pes%' AND umr.meta_value NOT LIKE '%admin%' WHERE s.timestamp >= DATE_SUB(NOW() - INTERVAL 1 DAY, INTERVAL 2 MONTH) GROUP BY s.timestamp, s.size ORDER BY s.timestamp ASC;
	*/

	if ( $stats ) {
		$content .= '<h3 class="center">Downloads per day</h3>';
		$content .= '<p class="center">(Last 30 days)</p>';
		$content .= '
      <div class="pes-chart">
    ...
    ';
		...
		$content   .= '<h3 class="center">Most downloaded photos</h3>';
		$content   .= '<p class="center">(All time)</p>';
		$sort_icon = '<i class="fa fa-sort" aria-hidden="true"></i>';
		$content   .= '
      <table class="table table-striped">
        <thead>
          <tr>
            <th><a href="?key=downloads&sort=' . $sort_inverted . '"><strong>Downloads ' . $sort_icon . '</strong></a></th>
      ...
      ';
		...
		foreach ( $stats as $line ) {
			$path_parts       = pathinfo( $line->attachment_url );
			$image_url        = $path_parts['dirname'] . '/' . $path_parts['filename'] . '-200x200.' . $path_parts['extension'];
			$gallery_post_url = "/gallery/$line->gallery_name";
			$content          .= '
          <tr>
            <td>' . $line->downloads . '</td>
            <td><a href="' . $gallery_post_url . '/' . $line->attachment_name . '"><img src="' . $image_url . '" alt="' . $line->attachment_title . '" title="' . $line->attachment_title . '" width="30" height="30"></a></td>
            <td><a href="' . $gallery_post_url . '">' . $line->gallery_title . '</a></td>
            <td>' . $line->size . '</td>
            <td>' . $line->country . '</td>
          </tr>
      ';
    ...
    ';
		...
		// Graphic data.
		$labels    = [];
		$daterange = new DatePeriod( new DateTime( "-1 month" ), new DateInterval( 'P1D' ), new DateTime() );
		foreach ( $daterange as $date ) {
			$tmp                     = $date->format( 'd M' );
			$labels['total'][ $tmp ] = $labels['web'][ $tmp ] = $labels['hr'][ $tmp ] = 0;
		}
		foreach ( $graph as $line ) {
			$labels[ $line->size ][ $line->dl_date ] += $line->downloads;
			$labels['total'][ $line->dl_date ]       += $line->downloads;
		}

		//echo '<pre>Labels '.print_r($labels,1).'</pre>';

		// Localize the script with graphic data.
		wp_localize_script( 'pes_funct_js', 'pes_ajax', [
			'labels' => array_keys( $labels['total'] ),
			'data'   => [
				'total' => array_values( $labels['total'] ),
				'web'   => array_values( $labels['web'] ),
				'hr'    => array_values( $labels['hr'] ),
			],
		] );
	} else {
...
function pes_download_attachment_redirect() {
	...
	if ( preg_match( '/^download-pes-photo\/(\d+)$/', $wp->request, $vars ) === 1 ) {
		...
		if ( get_post_type( $attachment_id ) === 'attachment' ) {
			if ( ! in_array( $vars[2], array( 'hr', 'web' ) ) ) {
				$vars[2] = 'web';
			}
...
function pes_download_attachment( $attachment_id = 0, $size = 'hr' ) {
	if ( get_post_type( $attachment_id ) === 'attachment' ) {
		...
		if ( $options['download_method'] === 'force' ) {
			...
			// New stats.
			pes_insert_download_data( array( $attachment_id ), $size );
...
function pes_download_gallery( $post_id, $photos, $size = 'hr' ) {
	if ( get_post_type( $post_id ) === 'gallery' ) {
		...
		// New stats.
		pes_insert_download_data( $photos, $size );
...
/**
 * Inserts the data for the stats for the given attachments and size.
 */
function pes_insert_download_data( $photos, $size ) {
	global $wpdb;
	$user = wp_get_current_user();
	...
	$wpdb->query( $wpdb->prepare( "
       INSERT INTO " . $wpdb->prefix . "pes_stats
       (attachment_id, user_id, size) VALUES
       $keys
      ", $values ) );

	/*$wpdb->insert(
	  $wpdb->prefix . 'pes_stats',
	  array(
