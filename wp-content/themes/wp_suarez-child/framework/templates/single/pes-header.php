...
<section class="pes-section">
    <div class="pes-header">
		<?php if ( ! $download ): ?>
            <div class="pes-header-left">
				...
				<?php if ( get_field( 'pes_name_of_meeting' ) ): ?>
                    <p class="pes-header-p pes-name-of-meeting"><?php the_field( 'pes_name_of_meeting' );
						echo $number_images; ?></p>
				<?php endif; ?>
				...
				<?php if ( $post->post_type == 'gallery' ): ?>
                    <form action="<?php echo get_permalink( $post_id ); ?>" method="get">
                        ...
                        <div class="pes-download">
                            <button type="submit" id="pes-download" class="pes-btn"><i class="fa fa-download"></i>Select
                                &amp; download photos
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="video-download">
						<?php if ( ! $user_ID ): ?>
							...
							<?php if ( function_exists( 'login_with_ajax' ) ) {
								login_with_ajax( $login_attributes );
							} ?>
						<?php else: ?>
                            <div class="size-btn">
								<?php foreach ( $videos as $k => $download ): ?>
                                    <div id="video-box" class="btn-container<?php if ( $k == 0 ) {
										echo ' pressed-video';
                            ...
                            <div id="video-attr"
                                 class="video-attr"><?php echo $videos[0]['width'] . ' x ' . $videos[0]['height'] . ' / ' . pes_formatBytes( $videos[0]['size'] ) ?></div>
                            <button type="button" id="video-download-btn" class="pes-btn"
                                    onClick="window.location.href='<?php echo $videos[0]['link'] ?>'"><i
                                        class="fa fa-download"></i>Download this video
                            </button>
						<?php endif; ?>
                    </div>
				<?php endif; ?>
		...
		<?php else: ?>
			<?php if ( $user_ID ): ?>
                <div class="pes-header-left">
                    <p><a href="<?php echo get_permalink( $post_id ); ?>">&laquo; Back to <?php the_title(); ?>
                            gallery</a></p>
                    <p>
                        <a id="selectAllPhotos"
                           href="javascript:void(0)"><?php echo 'Select all ' . count( $images ) . ' photos'; ?></a>
                        &#124;
